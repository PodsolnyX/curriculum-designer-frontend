...
const PlanAnalyticsPage = () => {
  ...
  const { data: statisticData, isLoading } = useGetStatisticsQuery(
    { curriculumId: Number(id), students: debouncedValue },
    { enabled: !!id },
  );
  ...
  const parsedData: PieChartData[] =
    statistic?.academicActivities?.map((row, index) => ({
      name: row.academicActivity?.name || 'Без названия',
      value:
        (row.rawHours / (statistic?.summary?.rawHours || 1) < 0.01
          ? statistic?.summary?.rawHours * 0.01
          : row.rawHours) || 0,
      color: COLORS?.[index],
    })) || [];

  const parsedResultData: PieChartData[] =
    statistic?.academicActivities?.map((row, index) => ({
      name: row.academicActivity?.name || 'Без названия',
      value:
        (row.resultHours / (statistic?.summary?.resultHours || 1) < 0.01
          ? statistic?.summary?.resultHours * 0.01
          : row.resultHours) || 0,
      color: COLORS?.[index],
    })) || [];
    ...
    >
      <div className={'flex flex-col gap-5 p-5'}>
        <div className={'flex gap-10 items-center'}>
          ...
          <div
            className={
              'flex flex-col align-center justify-center gap-2 shadow w-max p-3 px-8 rounded-lg'
            }
          >
            <Typography.Text type={'secondary'}>ППС ставки</Typography.Text>
            <Typography.Text className={'text-3xl'}>
              {statistic?.pps?.toFixed(3) || 0}
            </Typography.Text>
          </div>
        </div>
        <Table
          ...
          rowClassName={(record, index) =>
            index ===
            statistic?.academicActivities?.length +
              (statistic?.summary ? 1 : 0) -
              1
              ? 'bg-gray-50 font-bold'
              : ''
          }
          dataSource={
            statistic?.academicActivities
              ?.concat(statistic?.summary || [])
              .map((row) => ({
                ...row,
        ...
        <div className={'flex flex-wrap gap-5'}>
          <PieChartActivity data={parsedData} label={'Сумма часов по УП'} />
          <PieChartActivity
...
const PieChartActivity = ({ data, label }: PieChartProps) => {
  return (
    <div
      className={'w-full h-[300px] max-w-[530px] shadow rounded-md p-5 px-8'}
    >
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          ...
          <Pie
            ...
            labelLine={false}
            label={({ percent }) =>
              percent < 0.01 ? null : `${(percent * 100).toFixed(0)}%`
            }
            outerRadius={80}
            fill="#8884d8"
            dataKey="value"
