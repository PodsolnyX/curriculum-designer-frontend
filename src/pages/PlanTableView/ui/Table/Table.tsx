...
export const Table = () => {
  ...
  const getColumns = useCallback(() => {
    const columns = [
      ...
      columnHelper.accessor('index', {
        ...
        size: 100,
        cell: (props) => (
          <span className={'whitespace-nowrap text-[12px]'}>Без индекса</span>
        ),
      }),
      columnHelper.accessor('name', {
        ...
        cell: (props) => {
          ...
          return (
            <span
              className={`relative after:absolute after:h-[calc(100%+13px)] after:left-[-3px] after:top-[-7px] after:w-[2px] text-[12px] ${borderColor[props.row.original.type]}`}
            >
              {props.getValue()}
    ...
    columns.push(
      columnHelper.group({
        id: `attestation`,
        header: () => (
          <span className={'text-center'}>{`Формы пром. атт.`}</span>
        ),
    ...
    columns.push(
      columnHelper.group({
        ...
        header: () => (
          <span className={'text-center'}>{`Итого академ. часов`}</span>
        ),
    ...
    semestersData.forEach((semester) => {
      const academicActivityColumns = academicActivityData.map(
        (academicActivity) =>
          columnHelper.accessor(
            `${semester.semester.id}-${academicActivity.id}`,
            {
              header: () => academicActivity.shortName,
              cell: (props) => props.getValue(),
              size: 55,
      ...
      columns.push(
        columnHelper.group({
          ...
          header: () => (
            ...
            >{`Семестр ${semester.semester.number}`}</span>
          ),
          columns: [
            columnHelper.accessor(`${semester.semester.id}-attestation`, {
              ...
              cell: (props) =>
                props
                  .getValue()
                  ?.map((attestation) => attestation.shortName)
                  .join(', '),
              size: 50,
            }),
  ...
  const parseData = useMemo(() => {
      ...
      return atoms.map((atom) => {
        ...
        attestationTypesData.forEach((attestationType) => {
          total[`${attestationType.id}`] = atom.semesters
            .filter((semester) =>
              semester.attestations.find(
                (attestation) => attestation.id === attestationType.id,
              ),
            )
        ...
        academicActivityData.forEach((academicActivity) => {
          total[`activity-${academicActivity.id}`] = atom.semesters.reduce(
            (acc, semester) =>
              acc +
                semester.academicActivityHours.find(
                  (hours) => hours.academicActivity.id === academicActivity.id,
                )?.value || undefined,
            0,
          );
        });
        ...
        atom.semesters.forEach((semester) => {
          semester.academicActivityHours.forEach((hoursDistribution) => {
            semesters[
              `${semester.semester.id}-${hoursDistribution.academicActivity.id}`
            ] = hoursDistribution.value;
          });
          semesters[`${semester.semester.id}-attestation`] =
            semester.attestations;
        ...
        return {
          ...
          type: atom.type,
          credits: atom.semesters.reduce(
            (acc, semester) => acc + semester.credit,
            0,
          ),
          ...total,
          ...semesters,
      ...
      return modules.flatMap((module) => {
        ...
        attestationTypesData.forEach((attestationType) => {
          total[`${attestationType.id}`] = module.semesters
            .filter((semester) =>
              semester.nonElective.attestations?.find(
                (attestation) => attestation.id === attestationType.id,
              ),
            )
            .map((semester) => semester.semester.number);
        ...
        academicActivityData.forEach((academicActivity) => {
          total[`activity-${academicActivity.id}`] = module.semesters.reduce(
            (acc, semester) =>
              acc +
                semester.nonElective.academicActivityHours?.find(
                  (hours) => hours.academicActivity.id === academicActivity.id,
                )?.value || undefined,
            0,
          );
        ...
        module.semesters.forEach((semester) => {
          semester.nonElective.academicActivityHours?.forEach(
            (hoursDistribution) => {
              semesters[
                `${semester.semester.id}-${hoursDistribution.academicActivity.id}`
              ] = hoursDistribution.value;
        ...
        const rows: any = [
          {
            id: module.id,
            name: module.name,
            credits: module.semesters.reduce(
              (acc, semester) => acc + semester.nonElective.credit,
              0,
            ),
            ...total,
            ...semesters,
    ...
    return [...processAtoms(atomsData), ...processModules(modulesData)];
      ...
      >
        ...
        <tbody>
          {table.getRowModel().rows.map((row) => (
            <tr
              key={row.id}
              className={classNames(row.original?.rowColor && cls.moduleRow)}
            >
  ...
  return (
    <React.Fragment key={row.id}>
      {row.getVisibleCells().map((cell) => (
        <td
          key={cell.id}
          className={classNames(cls.textCenter, row?.rowColor && cls.moduleRow)}
      ...
      {row.getIsExpanded() && row.subRows?.length > 0 && (
        <td colSpan={row.getVisibleCells().length}>
          ...
          {/* Рекурсивный рендер для подстрок */}
        </td>
      )}
    </React.Fragment>
  );
