...
const RegisterPage = () => {
    ...
    const [data, setData] =
        useState<RegisterDto>({ email: "", password: "", firstName: "", lastName: "" });
        ...
        >
            <div className={"bg-white p-16 pb-24 rounded-xl max-w-[500px] w-full flex flex-col shadow-xl"}>
                ...
                <form className={"mt-12 flex flex-col"}>
                    ...
                    <Input size={"large"} className={"mb-2"} value={data.firstName} disabled={isPending}
                           onChange={(event) => setData({...data, firstName: event.target.value})}/>
                    <Typography.Text className={"text-lg"}>Фамилия</Typography.Text>
                    <Input size={"large"} className={"mb-2"} value={data.lastName} disabled={isPending}
                    ...
                    <Typography.Text className={"text-sm text-center text-stone-400 mt-2"}>
                        Есть аккаунт? <Link to={getRouteMain()} className={"text-sm"}>Войти в систему</Link>
                    </Typography.Text>
                </form>
            </div>
        </div>
