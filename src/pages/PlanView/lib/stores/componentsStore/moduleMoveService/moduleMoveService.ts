...
import {
  IMPOSSIBLE_MOVE_MODULE_INTO_SEMESTER_MESSAGE
...
export class ModuleMoveService {
  ...
  moveModule(activeId: string, overId: string) {
    ...
    const _indexInitialSemester = this.semesters.findIndex(s => s.id === activeSemesterId);
    const indexInitialSemester = module.semesters.findIndex(s => s.semester.id === activeSemesterId);

    const newIndexes = module.semesters.map((_, i) => indexTargetSemester + i - indexInitialSemester);
    ...
    runInAction(() => {
      ...
      this.updateModuleSemesters(module, newSemesters, overSemesterId, overParentModuleId);
      updatedAtoms = this.updateModuleAtoms(module, indexTargetSemester - _indexInitialSemester);
  ...
  private updateModuleSemesters(module: ModuleShortDto, newSemesters: SemesterDto[], overSemesterId: number, overParentModuleId: number) {
    module.parentModuleId = overSemesterId === overParentModuleId ? null : overParentModuleId;
  ...
  private updateModuleAtoms(module: ModuleShortDto, semestersDifference: number) {

    const updatedAtoms: Record<number, {semesterIds: Record<number, number>}> = {};

    module.atoms.forEach(atomId => {
      const atom = this.getAtom(atomId);
      const startedIndex = atom?.semesters[0].semester.number - 1 + semestersDifference;
      atom?.semesters.forEach((semester, i) => {
        updatedAtoms[atomId] = {semesterIds: {[semester.semester.id]: this.semesters[startedIndex + i].id}};
  ...
  private updateParentModulesAfterMove(moduleId: number, fromModuleId: number | null, toModuleId: number | null, newSemesters: SemesterDto[]) {
  ...
  private removeModuleFromParentModule(parentModule: ModuleShortDto, moduleId: number) {
    const updatedSemesters = parentModule.semesters.filter(modSem =>
      parentModule.modules
        ...
        .some(otherModuleId => {
          ...
          return otherModule?.semesters.some(s => s.semester.id === modSem.semester.id);
        })
    );

    if (!updatedSemesters.length) {
      const parentSemester = this.semesters.find(s => s.id === parentModule.parentSemesterId);
      if (parentSemester) {
  ...
  private mergeModuleIntoNewParentModule(parentModule: ModuleShortDto, moduleId: number, newSemesters: SemesterDto[]) {
    const mergedMap = new Map<number, SemesterDto>();

    const baseSemesters =
      parentModule.semesters.length === 1 && parentModule.semesters[0].semester.number === 0
        ? []
    ...
    parentModule.semesters = mergedArray.map(s => ({
      semester: s,
      nonElective:
        parentModule.semesters.find(sem => sem.semester.id === s.id)?.nonElective ?? {
          credit: 0,
          attestations: [],
          academicActivityHours: [],
        },
      elective:
        parentModule.semesters.find(sem => sem.semester.id === s.id)?.elective ?? {
          credit: 0,
          attestations: [],
          academicActivityHours: [],
        },
