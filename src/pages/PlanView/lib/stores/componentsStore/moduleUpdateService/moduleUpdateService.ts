import {
  AtomDto,
  CreateModuleDto,
  CreateModuleWithSelectionDto, CreateUpdateSelectionDto,
  SelectionDto,
  SemesterDto
...
export class ModuleUpdateService {
  ...
  async createModule(targetId: string, isSelection: boolean = false) {
    ...
    const newModule: ModuleShortDto = {
      ...
      index: null,
      selection: isSelection ? newSelectionParams.selection as SelectionDto : null,
    }
  ...
  updateModuleName(id: string, newName: string) {
    ...
    ModuleClient.updateModule(moduleId, {name: newName})
      .then(() => {
  ...
  transformModuleToSelection(id: string) {
    ...
    SelectionClient.createUpdateSelection(moduleId, newSelection)
      .then(() => {
        message.success(MODULE_SUCCESS_UPDATE_MESSAGE);
      })
  }
  ...
  transformSelectionToModule(id: string) {
    ...
    SelectionClient.deleteSelection(moduleId)
      .then(() => {
        message.success(MODULE_SUCCESS_UPDATE_MESSAGE);
      })
  }
  ...
  removeModule(id: string) {
    ...
    runInAction(() => {
      if (!!parentModule) {
        const moduleIndexInParent = parentModule.atoms
          .findIndex(childrenModuleId => childrenModuleId === moduleId);
    ...
    ModuleClient.deleteModule(moduleId)
      .then(() => {
