import { AtomDto, AtomType, CreateAtomDto, RefAtomSemesterDto, SemesterDto } from "@/api/axios-client.types.ts";
...
import {
  atomStrategyRegistry
} from "@/pages/PlanView/lib/stores/componentsStore/atomUpdateService/atomUpdateStrategy/AtomStrategyRegistry.ts";
...
export class AtomUpdateService {
  ...
  async createAtom(targetId: string) {
    ...
    const newAtom: AtomDto = {
      ...
      semesters: newAtomParams.semesterIds.map(semesterId => ({
        semester: this.semesters.find(semester => semester.id === semesterId) as SemesterDto,
        credit: 0,
        attestations: [],
    ...
    runInAction(() => {
      ...
      if (semesterId !== parentId) {
        const parentModule = this.modules.find(module => module.id === parentId);
        if (parentModule) {
  ...
  expandAtom(id: string, semesterNumber: number, direction: "prev" | "next") {
    ...
    const semester = this.semesters.find(semester => semester.number === semesterNumber);
  ...
  removeAtom(id: string) {
    ...
    if (getParentIdFromPrefix(id) !== getSemesterIdFromPrefix(id)) {
      parentModule = this.modules.find(module =>
        module.id === Number(getParentIdFromPrefix(id))
      )
    }

    runInAction(() => {
      if (!!parentModule) {
        const atomIndexInParent = parentModule.atoms
          .findIndex(moduleAtomId => moduleAtomId === atomId);
