...
export class CompetenceStrategy extends BaseAtomStrategy {
  updateLocal(atom: AtomDto, _semesterId: number, param: number[]) {
    const type =
      commonStore.curriculumData?.settings.competenceDistributionType;
  ...
  async updateRemote(atomId: number, _semesterId: number, param: number[]) {
    ...
    if (type === CompetenceDistributionType.Competence) {
      await AtomCompetenceClient.setAtomCompetences(atomId, {
        competenceIds: param,
      });
    } else {
      await AtomCompetenceClient.setAtomCompetenceIndicators(atomId, {
        competenceIndicatorIds: param,
      });
    }
