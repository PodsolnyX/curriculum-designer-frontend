...
import {
  ...
  IMPOSSIBLE_MOVE_ATOM_INTO_SEMESTER_MESSAGE,
...
export class AtomModuleMoveService {
  ...
  moveAtoms(activeId: string, overId: string) {
    ...
    runInAction(() => {
      this.updateAtomSemesters(
        atom,
        newSemesters,
        overSemesterId,
        overParentModuleId,
      );
      this.updateModulesAfterMove(
        atomId,
        activeParentModuleId,
        overParentModuleId,
        newSemesters,
      );
    });

    this.updateAtomServer(
      atomId,
      initialIndexes,
      newIndexes,
      overSemesterId,
  ...
  private updateModulesAfterMove(
    atomId: number,
    fromModuleId: number,
    toModuleId: number,
    newSemesters: SemesterDto[],
  ...
  private clearModuleFromEmptySemesters(module: ModuleShortDto) {
    ...
    if (!updatedSemesters.length) {
      const parentSemester = this.semesters.find(
        (s) => s.id === module.parentSemesterId,
      );
      if (parentSemester) {
  ...
  private mergeAtomIntoModule(
    ...
    module.semesters = mergedArray.map((s) => ({
      semester: s,
      nonElective: module.semesters.find((sem) => sem.semester.id === s.id)
        ?.nonElective ?? {
        credit: 0,
        attestations: [],
        academicActivityHours: [],
      },
      elective: module.semesters.find((sem) => sem.semester.id === s.id)
        ?.elective ?? {
        credit: 0,
        attestations: [],
        academicActivityHours: [],
