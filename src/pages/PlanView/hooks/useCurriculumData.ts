...
import {
  useGetCompetenceIndicatorsQuery,
...
export const useCurriculumData = (params: useCurriculumDataParams) => {
  const { atomsHasNoParentModule = false, modulesPlainList = false } = params;

  const { id } = useParams<{ id: string | number }>();

  const { data: curriculumData, isLoading: loadingPlan } =
    useGetCurriculumQuery({ id: Number(id) });
  const { data: semestersData, isLoading: loadingSemesters } =
    useGetSemestersQuery({ curriculumId: Number(id) });
  const { data: atomsData, isLoading: loadingAtoms } =
    useGetAtomsByCurriculumQuery({
      curriculumId: Number(id),
      hasNoParentModule: atomsHasNoParentModule,
    });
  const { data: modulesData, isLoading: loadingModules } =
    useGetModulesByCurriculumQuery({
      curriculumId: Number(id),
      plainList: modulesPlainList,
    });
  const { data: attestationTypesData, isLoading: loadingAttestationTypes } =
    useSearchAttestationsQuery();
  const { data: academicActivityData, isLoading: loadingAcademicActivity } =
    useGetAcademicActivitiesQuery({ curriculumId: Number(id) });
  const { data: indexesData, isLoading: loadingIndexes } = useGetIndexesQuery({
    curriculumId: Number(id),
  });
  const { data: competencesData, isLoading: loadingCompetences } =
    useGetCompetencesQuery({ curriculumId: Number(id) });
  const {
    data: competenceIndicatorsData,
    isLoading: loadingCompetenceIndicators,
  } = useGetCompetenceIndicatorsQuery(
    { curriculumId: Number(id) },
    {
      enabled:
        curriculumData?.settings.competenceDistributionType ===
        CompetenceDistributionType.CompetenceIndicator,
    },
  );
  const { data: validationErrorsData } = useGetValidationErrorsQuery({
    curriculumId: Number(id),
  });
  ...
  useEffect(() => {
    if (attestationTypesData)
      commonStore.setAttestationTypes(attestationTypesData);
  }, [attestationTypesData]);

  useEffect(() => {
    if (academicActivityData)
      commonStore.setAcademicActivity(academicActivityData);
  ...
  useEffect(() => {
    if (validationErrorsData)
      commonStore.setValidationErrors(validationErrorsData);
  }, [validationErrorsData]);

  useEffect(() => {
    if (loadingList.some((loading) => loading === true))
      commonStore.setIsLoadingData(true);
