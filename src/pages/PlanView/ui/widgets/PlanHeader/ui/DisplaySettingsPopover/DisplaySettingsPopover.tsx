...
import {
  DisplaySettingsList,
  PreDisplaySettings,
} from '@/pages/PlanView/const/preDisplaySettings.ts';
...
const DisplaySettingsPopover = observer(
  ({ children }: React.PropsWithChildren<{}>) => {
    const { id } = useParams<{ id: string }>();

    const selectedPreSetting =
      PreDisplaySettings.find(
        (setting) =>
          JSON.stringify(setting.settings) ===
          JSON.stringify(optionsStore.displaySettings),
      )?.key || '';
      ...
      return (
        <div className={'flex flex-col gap-3'}>
          <Select
            ...
            options={PreDisplaySettings.map((setting) => ({
              value: setting.key,
              label: setting.name,
            }))}
          ...
          <div className={'grid grid-cols-1 gap-1 mb-2'}>
            {DisplaySettingsList.map((setting) => (
              <Checkbox
                ...
                onChange={() =>
                  optionsStore.onChangeDisplaySetting(setting.key)
                }
                disabled={disabledEditSettings}
              >
                {setting.name}
