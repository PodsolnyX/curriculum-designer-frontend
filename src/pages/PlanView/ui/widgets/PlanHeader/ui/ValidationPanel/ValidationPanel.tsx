...
const ValidationPanel = observer(() => {
  ...
  const colorHoverBG = hasErrors
    ? 'hover:bg-red-500/[.2]'
    : 'hover:bg-green-500/[.2]';
  ...
  return (
    <div className="flex gap-5 items-center">
      <Tooltip title={message} placement={'left'}>
        <div
          className={`flex gap-1 items-center h-max ${colorBG} ${colorHoverBG} transition p-2 px-3 rounded-lg cursor-pointer`}
          onClick={() =>
        ...
        >
          <Typography.Text className={colorText}>
            {commonStore.validationErrors?.length}
          </Typography.Text>
