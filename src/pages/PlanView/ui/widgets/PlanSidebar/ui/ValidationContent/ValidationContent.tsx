...
const ValidationContent = observer(() => {
  ...
  const scrollToTarget = (error: ValidationError) => {
    ...
    const targetId = error.entities?.reduce((prev, current) => {
      return prev
        ? concatIds(
            setPrefixToId(Number(current.id), Entities[current.type]),
            prev,
          )
        : setPrefixToId(Number(current.id), Entities[current.type]);
    }, '');
  ...
  return (
    <div className={'flex flex-col gap-2 h-full'}>
      ...
      {data?.length ? (
        <ul className={'flex flex-1 flex-col overflow-y-auto scrollbar'}>
          {data?.map((error, index) => (
            <li
              key={index}
              className={
                'flex flex-col bg-white hover:bg-stone-100 p-2 transition rounded-md cursor-pointer'
              }
        ...
        <Typography.Text
          className={'text-stone-400 mt-28 whitespace-break-spaces text-center'}
        >
