...
const AddCompetencePopover = observer(
  ({
    subjectId,
    competencies,
    competenceDistributionType,
    onChange,
  }: AddCompetencePopoverProps) => {
    const [selectedType, setSelectedType] = useState<string>(
      CompetenceTypeName[CompetenceType.Basic].shortName,
    );
    ...
    useEffect(() => {
      setSelectedCompetence(
        commonStore.competencesTree
          .filter(
            (competence) =>
              CompetenceTypeName[competence.type].shortName === selectedType,
          )
          .filter(
            (competence) =>
              competence.name.toLowerCase().indexOf(search.toLowerCase()) !==
              -1,
          ),
      );
    }, [selectedType, search]);
      ...
      <div className={'flex flex-col gap-1'}>
        <Segmented
          options={Object.values(CompetenceType).map(
            (type) => CompetenceTypeName[type].shortName,
          )}
          value={selectedType}
        ...
        />
        <div
          className={'flex flex-col max-h-[300px] overflow-y-auto scrollbar'}
        >
          {selectedCompetence.length ? (
            ...
            <span className={'text-stone-400 text-sm pt-2'}>
