import {CompetenceDistributionType, CompetenceDto} from "@/api/axios-client.types.ts";
...
const CompetenceItem = memo(({id, name, index, indicators, competencies, competenceDistributionType, onChange}: CompetenceItemProps) => {
    ...
    const countSelectedIndicators = indicators
        .reduce((prev, indicator) =>
            prev + Number(!!competencies.find(competence => competence === indicator.id)), 0)
