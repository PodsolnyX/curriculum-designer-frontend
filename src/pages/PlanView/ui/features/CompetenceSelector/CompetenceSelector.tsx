...
const CompetenceSelector = observer(
  ({
    competencies = [],
    size = 'small',
    subjectId,
    ...
    return (
      <div
        className={`flex flex-wrap gap-1 max-h-[150px] overflow-y-auto scrollbar group items-center ${!competencies.length ? 'justify-between' : ''}`}
      ...
      >
        {competencies?.length ? (
          ...
          <span
            className={`${size === 'small' ? 'text-[10px]' : 'text-[12px]'} text-stone-400`}
          >
            Нет компетенций
          </span>
        )}
        ...
        >
          <Tag
            color={'default'}
            className={
              'm-0 group-hover:opacity-100 opacity-0 cursor-pointer px-5 text-center text-stone-400 hover:text-black'
            }
            bordered={size !== 'small'}
          >
            +
...
const CompetenceTag = observer(
  ({ competence, size = 'small', onRemoveCompetence }: CompetenceTagProps) => {
    ...
    return (
      <Tag
        ...
        key={competence}
        onClick={() =>
          commonStore.selectCompetence(!isSelected ? competence : null)
        }
      >
        <Tooltip title={competenceInfo?.description}>
          <span
            className={`${size === 'small' ? 'text-[10px]' : 'text-[14px]'}`}
          >
        ...
        <Tooltip title={'Удалить'}>
          <span
            ...
            className={
              'text-[12px] cursor-pointer text-stone-300 hover:text-stone-500 hidden group-hover/item:flex'
            }
          >
            ×
          </span>
