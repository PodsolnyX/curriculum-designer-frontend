...
const ValidationTab = () => {
  ...
  const { data: validators, isLoading } = useGetValidatorsQuery(
...
const ValidatorItem = ({ validator }: ValidatorItemProps) => {
  return (
    <div
      className={
        'flex flex-wrap items-center rounded-md p-2 group gap-4 w-full hover:bg-stone-50'
      }
    >
      <Checkbox checked={validator.isEnabled} />
      <div className={'flex flex-col flex-1 min-w-[300px]'}>
        {ValidationLevelDisplay[validator.level]}
        <Typography.Text>{validator.name}</Typography.Text>
      </div>
    </div>
  );
};
