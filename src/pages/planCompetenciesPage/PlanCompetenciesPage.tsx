...
import CompetenceItem from "@/pages/planCompetenciesPage/CompetenceItem.tsx";
...
const PlanCompetenciesPage = () => {
    ...
    const [activeCompetence, activeIndicator] = useMemo(
        () => {
            ...
            const indicatorCompetence = items.find((item) => item.id !== active?.id && item?.indicators.find(indicator => indicator.id === active?.id))
            return [null, indicatorCompetence?.indicators.find(indicator => indicator.id === active?.id)]
        },
        [active, items]
    );
