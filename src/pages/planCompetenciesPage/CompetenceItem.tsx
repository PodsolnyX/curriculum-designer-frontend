import {CompetenceDto, CompetenceIndicatorDto, CompetenceType} from "@/api/axios-client.types.ts";
...
import {DeleteOutlined, DownOutlined, HolderOutlined, LeftOutlined, MoreOutlined} from "@ant-design/icons";
import AddIndicatorButton from "@/pages/planCompetenciesPage/AddIndicatorButton.tsx";
...
const CompetenceItem = ({name, index, indicators: _indicators, id, type}: CompetenceDto) => {
    ...
    const [indicators, setIndicators] = useState<CompetenceIndicatorDto[]>(_indicators);
                                     ...
                                     dataSource={[
                                         {
                                             ...
                                             children:
                                                 <List
                                                     size={"small"}
                                                     itemLayout={"vertical"}
                                                     dataSource={Object.values(CompetenceType).map(type => {return { key: type, label: CompetenceTypeName[type].name}})}
                                                     renderItem={(item) =>
                                                         <li className={"w-full"}>
                                                             <Button
                                                                 type={"text"}
                                                                 ...
                                                                 onClick={() => changeType(item.key as CompetenceType)}
                                                             >{item.label}</Button>
                                                         </li>
                                                     }
                                                 />,
                                         ...
                                         { key: 'delete', label: 'Удалить', danger: true, icon: <DeleteOutlined/>, onClick: () => deleteCompetence() }
                                     ]}
                                     renderItem={(item) =>
                                         <li className={"w-full"}>
                                             {
                                                 item.children ?
                                                     <Popover content={item.children} trigger={"click"} placement={"left"}>
                                                         <Button
                                                             type={"text"}
                ...
                (showIndicators && !isDragging) ?
                    ...
                    >
                        <div className={"flex flex-col gap-2 border-l border-stone-100 ml-2 mt-2"}>
                            ...
                            <AddIndicatorButton competenceId={id} curriculumId={Number(curriculumId) || 0}/>
                        </div>
                    </SortableContext>
                    : null
            }
